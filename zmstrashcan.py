...
class ZMSTrashcan(ZMSContainerObject):
    ...
    def manage_changeProperties(self, lang, manage_lang, REQUEST=None): 
      ...
      if REQUEST.get('btn','') in  [ self.getLangStr('BTN_CANCEL',manage_lang), self.getLangStr('BTN_BACK',manage_lang)]:
        return REQUEST.RESPONSE.redirect('manage_main?lang=%s&manage_lang=%s'%(lang,manage_lang))
      ...
      if REQUEST and hasattr(REQUEST,'RESPONSE'):
        if REQUEST.RESPONSE:
          return REQUEST.RESPONSE.redirect('manage_properties?lang=%s&manage_lang=%s&manage_tabs_message=%s'%(lang,manage_lang,urllib.quote(message)))
