...
def recurse_updateVersionBuild(docElmnt, self, REQUEST):
  ...
  ##### Build 124a: Remove sort_ids from object-versions. ####
  if getattr( docElmnt, 'build', '000') < '124':
    if self.version_live_id is not None:
      setattr( self, 'sort_id', getattr( getattr( self, self.version_live_id), 'sort_id', 's0000'))
    for ob_vers in self.objectValues( ['ZMSAttributeContainer']):
      try:
        delattr( ob_vers, 'sort_id')
      except:
        pass

  ##### Build 125a: Remove work-versions. ####
  if getattr( docElmnt, 'build', '000') < '125':
    if len( getObjStates( self)) == 0 and self.version_live_id is not None and self.version_work_id in self.objectIds(['ZMSAttributeContainer']):
      ids = []
    ...
    for rubrik in self.objectValues( ['ZMSRubrik']):
      ...
      for key in rubrik.__dict__.keys():
        if key != 'id' and hasattr( rubrik, key):
          ...
          if key == '__work_state__' or v is None or type( v) is type( ''):
        ...
        for key in self.getObjAttrs().keys():
          ...
          if datatype in _globals.DT_STRINGS:
            for lang in self.getLangIds():
              ...
              if data is not None and type( data) is type( '') and data != '':
                changed = False
                while True:
                  i = data.find( 'ZMSRubrik')
                  if i < 0:
      ...
      for ob_vers in self.objectValues( ['ZMSAttributeContainer']):
        ...
        if hasattr( ob_vers, key):
          try:
            ...
            pass 
    if changed:
      print self, "restricted=", self.hasRestrictedAccess(), "attr_dc_accessrights_restricted=", 'attr_dc_accessrights_restricted' in self.getObjAttrs().keys()
      self.synchronizePublicAccess()
...
def recurse_updateVersionPatch(docElmnt, self, REQUEST):
  ...
  ##### Build 125k: ZMSFolder. ####
  if getattr( docElmnt, 'build', '000') > '000' and \
     getattr( docElmnt, 'build', '000') <= '125' and \
     getattr( docElmnt, 'patch', '000') < 'k':
    self.setConfProperty('ZMSFolder.enabled',self.getConfProperty('ZMSFolder.enabled',0))
...
def initZMS(self, titleshort, title, lang, manage_lang, REQUEST):
  ...
  for id in root.objectIds(['DTML Method']):
    if id.find( 'defaultCSS_') == 0 or id[-3:] == 'CSS':
...
def manage_addZMS(self, lang, manage_lang, REQUEST, RESPONSE):
  ...
  if REQUEST['btn'] == 'Add':
    ...
    if REQUEST.get('initialization',0)==2:
      ...
      lcms = initZMS(lcmsHomeElmnt,titlealt,title,'ger','ger',REQUEST)
...
class ZMS(
    ...
    __authorPermissions__ = (
		...
		'manage_wait',
		'manage_addZMSFolder','manage_addZMSDocument','manage_addZMSTextarea','manage_addZMSGraphic','manage_addZMSTable','manage_addZMSFile','manage_addZMSTeaserContainer','manage_addZMSNote','manage_addZMSLinkContainer','manage_addZMSLinkElement','manage_addZMSSqlDb',
    ...
    def getPortalMaster(self):
      ...
      if len(v) > 0:
        try:
          masterHome = getattr( self, v, None)
    ...
    #  Update version.
    # --------------------------------------------------------------------------
    def updateVersion(self, lang, manage_lang, REQUEST, maintenance=True):
      message = ''
      ...
      for portalClient in self.getPortalClients():
        message += portalClient.updateVersion( lang, manage_lang, REQUEST, False)
