...
class ZMSFile(ZMSObject, Metadata): 
    ...
    def manage_changeProperties(self, lang, manage_lang, REQUEST, RESPONSE):
      ...
      # Return with message.
      if redirect_self:
        return RESPONSE.redirect('manage_main?lang=%s&manage_lang=%s&%s=%s'%(lang,manage_lang,messagekey,urllib.quote(message)))
      else:
        self.checkIn(REQUEST)
        return RESPONSE.redirect('%s/manage_main?lang=%s&manage_lang=%s&%s=%s#_%s'%(self.getParentNode().absolute_url(),lang,manage_lang,messagekey,urllib.quote(message),self.id))
