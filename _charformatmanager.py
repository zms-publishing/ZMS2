...
class CharFormatManager: 
    ...
    #  Change char-formats.
    ############################################################################
    def manage_customizeCharFormat(self, lang, manage_lang, REQUEST, RESPONSE): 
      """ CharFormatManager.manage_customizeCharFormat """
      message = ''
      id = REQUEST.get('id','')
      ...
      # Delete.
      # -------
      elif REQUEST['btn'] in [ self.getLangStr('BTN_DELETE',manage_lang), 'delete']:
        id = int(id)
      ...
      # --------
        ...
        message = self.getLangStr('MSG_MOVEDOBJTOPOS', manage_lang)%(("<i>%s</i>"%str(id)),(pos+1))
        id = ''

      # Return with message.
      message = urllib.quote(message)
      return RESPONSE.redirect('manage_customizeTextFormatForm?lang=%s&manage_lang=%s&manage_tabs_message=%s&id=%s#form1'%(lang,manage_lang,message,str(id)))
