...
class ZReferableItem: 
  ...
  manage_browse_objs = browse_objs = HTMLFile('dtml/zmslinkelement/browse_objs', globals()) 
    ...
    if isInternalLink(url):
      ...
      if ob is not None:
        if ob.meta_type == 'ZMSFile':
          ...
          if file is not None:
            return file.getHref(REQUEST)
        else:
          return ob.getHref2IndexHtml(REQUEST)
      else:
        return './index_%s.html?op=not_found&url=%s'%(REQUEST.get('lang',self.getPrimaryLanguage()),url)
    elif isMailLink (url): 
      prefix = 'mailto:'
  ...
  def synchronizeRefs( self, ob_id=None):
    ...
    for abs_url in abs_urls:
      ...
      # Process recordset.
      if ob.meta_type == 'ZMSCustom' and ob.getType()=='ZMSRecordSet':
                  ...
                  if l > 1:
                    ...
                    for i in l[1:]:
                      ...
                      if len( f) == 0:
                        ...
                        ref = ob.getRefObjPath( obs[f[0]])[2:-1]
                      if ref.startswith('__') and ref.endswith('__'):
