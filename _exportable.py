...
def localHtml(self, obj, html, REQUEST):
  ...
  for js in [ 'comlib', 'formlib', 'zmilib', 'styleswitcher']:
...
class Exportable:
    ...
    def exportContentRessources(self, tempfolder, REQUEST, incl_embedded):
      ...
      if REQUEST is not None:
        ...
        #-- JavaScript
        for js in [ 'comlib', 'formlib', 'formlib', 'zmilib', 'styleswitcher']:
      ...
      if not self.meta_type == 'ZMS' and not REQUEST.get( 'ZMS_FILTER', False):
        base_path += self.id+'/'
      return _blobfields.recurse_downloadRessources( self, base_path, REQUEST, incl_embedded)
