...
class ZMSContainerObject(
    ...
    def manage_ajaxDragDrop( self, lang, target, REQUEST, RESPONSE):
      ...
      try:
        ...
        ob.getParentNode().manage_pasteObjects(cb_copy_data)
        ob.getParentNode().normalizeSortIds()
      except:
    ...
    def getLinkList(self, REQUEST, allow_none=0):
      print "[ZMSContainerObject.getLinkList]: @deprecated: use implementation that fits your own needs!"
    ...
    def getNavItems(self, current, REQUEST, opt={}):
      items = []
      if not self.meta_type in ['ZMSDocument','ZMSCustom']:
        items.append( '<ul')
        if opt.get('id',''):
      ...
      for ob in obs:
        if not ob.isResource(REQUEST):
          ...
          items.append('</a>')
          if opt.get('complete',False):
            items.append( ob.getNavItems( current, REQUEST, opt))
          elif opt.get('deep',True) and ob.id != self.id and ob.id in current.getPhysicalPath():
            items.append( ob.getNavItems( current, REQUEST, opt))
          items.append('</li>\n')
      if not self.meta_type in ['ZMSDocument','ZMSCustom']:
        items.append( '</ul>\n')
    ...
    def getTeaserElements(self, REQUEST, current=None):
      ...
      for teaserCntnr in self.filteredChildNodes( REQUEST,[ 'ZMSTeaserContainer']):
        thisTeaserElmnts.extend( teaserCntnr.filteredChildNodes( REQUEST,['ZMSTeaserElement','ZMSFile','ZMSCustom']))
