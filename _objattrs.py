...
class ObjAttrs:
    ...
    def getObjAttrInput(self, fmName, obj_attr, value, REQUEST):
      ...
      lang_str = self.getZMILangStr(label.upper())
      ...
      #-- Richtext-Fields.
        ...
        return self.f_selectRichtext(self,ob=self,fmName=fmName,elName=elName,value=value,key=obj_attr['key'],enabled=enabled,lang=lang,REQUEST=REQUEST)
      ...
      #-- Image-Fields.
        ...
        return self.f_selectImage(self,ob=self,fmName=fmName,elName=elName,value=value,key=obj_attr['key'],lang=lang,REQUEST=REQUEST)
      
      #-- File-Fields.
      elif inputtype == 'file':
        return self.f_selectFile(self,ob=self,fmName=fmName,elName=elName,value=value,key=obj_attr['key'],lang=lang,REQUEST=REQUEST)
      ...
      #-- Text-Fields.
        ...
        if inputtype == 'dialog':
          try:
            ...
            inp = []
            inp.append('<div class="form-element"><i>%s</i></div>'%(self.getZMILangStr('MSG_AFTER_INSERT')%self.display_type(REQUEST,ins_meta_type)))
        return ''.join(inp)
    ...
    def getObjProperty(self, key, REQUEST={}, par=None):
      ...
      try:
        ...
        elif key not in self.dGlobalAttrs.get(self.meta_type,{}).get('objAttrs',{}).keys() and key in self.getMetadictAttrs( self.meta_type):
          try:
        ...
        if metaObjAttr is not None and metaObjAttr['type'] == 'method':
          ...
          if value is not None:
            value = value._createCopy( self, key)
            value.lang = None
        ...
        #-- Standard attributes.
          ...
          if datatype == _globals.DT_TEXT:
            if type(value) in StringTypes and value.find('<dtml-')>=0:
              if self.meta_type == 'ZMSCustom' and REQUEST.get('indexBtn','') != '' and value.find('indexBtn') > 0:
          ...
          for meta_id in self.getMetaobjIds(sort=0):
            ...
            for key in self.getMetaobjAttrIds(meta_id):
              ...
              try:
                ...
                _globals.writeException( self, '[synchronizeObjAttrs]: key=%s'%key)
