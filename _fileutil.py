...
def getOSPath(path, sep=None, chs=range(32)+[34,60,62,63,127]):
  ...
  path = path.encode('ascii', 'replace') # replace => uncodierbare Zeichen durch ? (63) ersetzen 
  if len( chs) > 0:
    for ch in chs:
...
def resizeImage(img, perc):
  ...
  # Save image in temp-folder.
  tempfolder = tempfile.mktemp()
  filename = urllib.quote(getOSPath('%s/%s'%(tempfolder,img.filename)), '/\:')
  exportObj(img,filename)
...
#  OUT: resized
# ------------------------------------------------------------------------------
def resizeImageXY(img, width, height):
...
# ------------------------------------------------------------------------------
def _buildZipArchive( zf, folder, path, filter):
  for file in os.listdir( folder + os.sep + path):
    filepath = folder + os.sep + path + os.sep + file
    mode = os.stat( filepath)[stat.ST_MODE]
    if stat.S_ISDIR( mode): 
      _buildZipArchive( zf, folder, filepath[ len( folder + os.sep): ], filter)
    else:
      if fnmatch.fnmatch( extractFilename( filepath), filter):
        zf.write( filepath, filepath[ len( folder + os.sep): ])
