...
def manage_addZMSSysFolder(self, lang, _sort_id, REQUEST):
  ...
  if REQUEST['btn'] == self.getZMILangStr('BTN_INSERT'):
    ...
    if REQUEST.RESPONSE:
      REQUEST.RESPONSE.redirect('%s/manage_main?lang=%s'%(self.absolute_url(),lang))
...
class ZMSSysFolder(ZMSContainerObject):
    ...
    # Properties.
    # -----------
    meta_type = meta_id = "ZMSSysFolder"
