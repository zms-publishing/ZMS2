...
def getHref2Zoom(self, img, imgattr, max_width, max_height, REQUEST):
  ...
  try:
    ...
    href += "javascript:open_function('f_zoomImage?src=%s',%i,%i,'%s')"%(img.getHref(REQUEST),width,height,options)
  except:
...
class ObjTypes:
    ...
    def renderDisplaytype(self, displaytype, \
      ...
      if img is None or img.get_size() == 0:
        ...
        height = ''
        try:
          max_width = self.getConfProperty('ZMSGraphic.zmi_max_width',400)
