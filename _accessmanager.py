...
class AccessableObject: 
    ...
    def synchronizePublicAccess(self):
      ...
      if self.meta_type == 'ZMSLinkElement' and self.isEmbedded( self.REQUEST):
        ob = self.getRefObj()
        if ob is not None:
...
class AccessManager(AccessableContainer): 
    ...
    def manage_userProperties(self, btn, key, lang, manage_lang, REQUEST, RESPONSE=None):
      ...
      # Cancel.
      # -------
      if btn in [ self.getLangStr('BTN_CANCEL',manage_lang), self.getLangStr('BTN_BACK',manage_lang)]:
        id = ''
