...
def manage_addZMSTable(self, lang, _sort_id, REQUEST, RESPONSE):
  """ manage_addZMSTable """
...
class ZMSTable(ZMSObject):
    ...
    __obj_attrs__ = {
        ...
        'align':{'datatype':'string', 'type':'select','default':'TOP','options':['TOP','TOP','BOTTOM','BOTTOM','LEFT','LEFT','RIGHT','RIGHT'], 'mandatory': True},
        # Meta-Data
    ...
    def manage_changeProperties(self, lang, REQUEST, RESPONSE): 
        ...
        if len(REQUEST.get('function','')) > 0 or \
           REQUEST.get('btn','') not in [ self.getZMILangStr('BTN_CANCEL'), self.getZMILangStr('BTN_BACK')]:
          ...
          table = copy.deepcopy(self.getTable( REQUEST))
        ...
        for row in table:
          ...
          for cell in row:
            if rowindex == 0 and colindex == 0 and type == 4:
              ...
              text = self.renderText( textformat, 'table_%i_%i'%(colindex, rowindex), text, REQUEST)
              try:
