...
def isInternalLink(url):
  rtn = type(url) is type('') and len(url)>=3 and url[:2]=='{$' and url[-1]=='}'
  return rtn
      ...
      if url.find('{$__') != 0:
        ...
        if docElmnt is not None:
          try:
            ...
            _globals.writeException(self,'[getLinkObj]')
        if ob is None:
          _globals.writeLog( self,'[getLinkObj]: Invalid internal url %s'%url)
    return ob
