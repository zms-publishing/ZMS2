...
class ZMSGlobals: 
    ...
    def FileFromData( self, data, filename=''):
      return _blobfields.createBlobField( self, _globals.DT_FILE ,file={'data':data,'filename':filename})

    # --------------------------------------------------------------------------
    #  ZMSGlobals.ImageFromData:
    # --------------------------------------------------------------------------
    def ImageFromData( self, data, filename=''):
      return _blobfields.createBlobField( self, _globals.DT_IMAGE ,file={'data':data,'filename':filename})
    ...
    # --------------------------------------------------------------------------
    def encrypt_password(self, pw, algorithm='md5'):
      enc = None
      if algorithm.upper() == 'MD5':
        import md5
        enc = md5.new(pw).digest()
    ...
    # --------------------------------------------------------------------------
    def string_maxlen(self, s, maxlen=20, etc='...'):
      if len(s) > maxlen:
        s = s[:maxlen] + etc
      return s
    ...
    # --------------------------------------------------------------------------
    def string_replaceall(self, s, old, new):
      while True:
        i = s.find( old)
        if i < 0: 
          break
    ...
    def url_append_params(self, url, dict):
      ...
      for key in dict.keys():
        value = dict[key]
        url += sep + key + '=' + urllib.quote(str(value))
        sep = '&'
      url += targetdef
      return url
    ...
    # --------------------------------------------------------------------------
    def re_sub( self, regex, replacement, subject):
    ...
    # --------------------------------------------------------------------------
    def parse_stylesheet(self, REQUEST=None):
      stylesheet = self.getStylesheet()
      # Remove comments
      if REQUEST is None:
        data = stylesheet.raw
      else:
        data = stylesheet( stylesheet, REQUEST)
      newData = ''
      ...
      for elmnt in data.split('}'):
        i = elmnt.find('{')
        keys = elmnt[:i].strip()
        value = elmnt[i+1:].strip()
        for key in keys.split(','):
          key = key.strip()
          if len(key) > 0:
            styleDict[key] = styleDict.get(key,'') + value
      # Return dictionary
      return styleDict
    ...
    def get_colormap(self):
      styleDict = self.parse_stylesheet()
      colorMap = {}
    ...
    # --------------------------------------------------------------------------
    def getLangFmtDate(self, t, lang='eng', fmt_str='DATETIME_FMT'):
      try:
