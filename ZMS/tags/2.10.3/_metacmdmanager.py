...
# $Revision: 1.7 $
#
# Implementation of class MetacmdManager (see below).
#
...
class MetacmdObject: 
    ...
    ############################################################################
    # MetacmdManager.manage_executeMetacmd:
    #
    # Execute Meta-Command.
    ############################################################################
    def manage_executeMetacmd(self, custom, lang, manage_lang, REQUEST, RESPONSE):
      """ MetacmdManager.manage_executeMetacmd """
      message = ''
      ...
      for metaCmdId in self.getMetaCmdIds():
        ...
        if metaCmd['name'] == custom:
          ...
          # Execute redirect.
            ...
            return RESPONSE.redirect('%s?lang=%s&manage_lang=%s'%(metaCmd['id'],lang,manage_lang))
      
      # Return with message.
      message = urllib.quote(message)
      return RESPONSE.redirect('manage_main?lang=%s&manage_lang=%s&manage_tabs_message=%s'%(lang,manage_lang,message))
...
class MetacmdManager: 
    ...
    ############################################################################
    def manage_changeMetacmds(self, btn, lang, manage_lang, REQUEST, RESPONSE):
        """ MetacmdManager.manage_changeMetacmds """
        message = ''
        id = REQUEST.get('id','')
        ...
        # -----
          ...
          if metaOb.get('acquired',0) == 1:
            ...
            if portalMaster is not None:
              REQUEST.set('ids',[id])
              xml =  portalMaster.manage_changeMetacmds(self.getLangStr('BTN_EXPORT',manage_lang), lang, manage_lang, REQUEST, RESPONSE)
	  ...
	  if f:
        ...
        # Return with message.
        message = urllib.quote(message)
        return RESPONSE.redirect('manage_customizeMetacmdForm?lang=%s&manage_lang=%s&manage_tabs_message=%s&id=%s'%(lang,manage_lang,message,id))
