...
def recurse_downloadRessources(self, base, folder='.', deep=True):
  ...
  for key in obj_attrs.keys():
    ...
    if datatype in _globals.DT_BLOBS:
      for lang in base.getLangIds():
        try:
          if obj_attr['multilang']==1 or lang==self.getPrimaryLanguage() or (obj_attr['multilang']==0 and lang!=self.getPrimaryLanguage()):
...
class MyBlob:
    ...
    def __call__(self, REQUEST=None, **kw):
      ...
      if REQUEST is not None and REQUEST.has_key('path_to_handle'):
        ...
        if not parent.hasAccess( REQUEST) and not parent.getConfProperty( 'ZMS.blobfields.grant_public_access', 0) == 1:
        ...
        public_access = parent.hasPublicAccess()
        cached_page = parent.isCachedPage( REQUEST)
        if not public_access and not cached_page:
