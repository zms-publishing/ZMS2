...
class ZMSContainerObject(
    ...
    # --------------------------------------------------------------------------
    #  ZMSContainerObject.zms_type:
    # --------------------------------------------------------------------------
    def zms_type(self):
      return "ZMSContainerObject"
    ...
    ############################################################################
    def manage_undoObjs(self, lang, manage_lang, ids, REQUEST, RESPONSE=None):
      """ ZMSContainerObject.manage_undoObjs """ 
      ...
      # Return with message.
      if RESPONSE is not None:
        message += self.getLangStr('MSG_UNDONE',manage_lang)%c
        message += ' (in '+str(int((time.time()-t0)*100.0)/100.0)+' secs.)'
        target = REQUEST.get('target','manage_main')
    ...
    def filtered_metapattern_actions(self, path, REQUEST):
      ...
      #-- Headline,
      if len(actions) > 0:
        actions.insert(0,('----- %s'%self.getLangStr('CAPTION_INSERT', manage_lang)%self.getLangStr('TAB_METAPATTERN',manage_lang),''))
    ...
    def filtered_insert_actions(self, path, REQUEST):
      ...
      for meta_id in self.getMetaobjIds():
        ...
        if can_insert:
          if (self.meta_type in ['ZMS','ZMSFolder'] and ob['type'] in ['ZMSDocument','ZMSObject','ZMSResource','ZMSRecordSet','ZMSReference']) or \
    ...
    def getChildNodes(self, REQUEST=None, meta_types=None):
      ...
      if _globals.debug( self): _globals.writeLog( self, "[getChildNodes] <<<<< OUT (%s)"%str(time.time()-t0))
