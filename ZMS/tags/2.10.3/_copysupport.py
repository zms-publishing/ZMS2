        ...
        if forced or id in ids:
          ...
          if _id is not None:
            ...
            if new_id != _globals.id_prefix(id): 
              ...
              if _globals.debug( self): 
                _globals.writeLog( self, '[_normalize_ids_after_copy]: %s.%s(%s) %s'%(self.id,id,ob.meta_type,new_id))
          ...
          if _id is not None:
            ...
            if new_id != _globals.id_prefix(id): 
              if new_id == 'e':
                new_id = self.getNewId(new_id)
              if _globals.debug( self): 
                _globals.writeLog( self, '[_normalize_ids_after_move]: %s.%s(%s) %s'%(self.id,id,ob.meta_type,new_id))
              self.manage_renameObject(id=id,new_id=new_id)
              id = new_id
