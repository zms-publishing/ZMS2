...
# DEPRECATED!
class MyDict(Persistent,Implicit): pass    
...
# ------------------------------------------------------------------------------
#  _globals.filename_quote:
# ------------------------------------------------------------------------------
def filename_quote(filename):
  for ch in ['+','%',' ','!','?','#','"']:
...
def id_quote(srctext):
  ...
  for key in mapping.keys():
    dsttext = dsttext.replace(key,mapping[key])
  for ch in range(256):
    if not (ch in [ord('_')] or \
    	    ch in range(ord('0'),ord('9')+1) or \
    	    ch in range(ord('A'),ord('Z')+1) or \
    	    ch in range(ord('a'),ord('z')+1)):
      dsttext = dsttext.replace(chr(ch),'')
  dsttext = dsttext.lower()
  return dsttext
...
def dt_html(self, value, REQUEST): 
  ...
  while True:
    ...
    i = j
  while True:
    if value.find( '</dtml-var>') >= 0:
      value = value.replace( '</dtml-var>', '')
    else:
      break
...
#  _globals.nvl:
#
#  Returns its first argument if it is not None, otherwise it returns its second argument.
# ------------------------------------------------------------------------------
def nvl(a1, a2):
  if a1 is not None:
    return a1
...
# ------------------------------------------------------------------------------
#  _globals.isCacheRequest:
#
#  Returns true if current context is cache-request, false else.
# ------------------------------------------------------------------------------
def isCacheRequest(REQUEST):
  return not isPreviewRequest(REQUEST) and REQUEST.get('__cache__',0) == 1
...
def writeLog(self, info):
  try:
    ...
    if 'DEBUG' in zms_log.logged_entries:
      ...
      info = "[%s@%s]"%(self.meta_type,self.absolute_url()[len(self.REQUEST['SERVER_URL']):]) + info
