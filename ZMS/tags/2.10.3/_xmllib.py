...
# ---------------------------------------------------------------------------------------------
def getAttrToXml(self, base, base_path, obj_attr, REQUEST):
  xml = ''
  ...
  if value is not None:
    ...
    # Objects.
    if datatype in _globals.DT_BLOBS:
      xml += value.toXml( self, base, base_path)
...
# ---------------------------------------------------------------------------------------------
def getObjPropertyToXml(self, base, base_path, obj_attr, REQUEST):
  xml = ''
  # Multi-Language Attributes.
  if obj_attr['multilang']:
    ...
    for langId in langIds:
      ...
      s_attr_xml = getAttrToXml( self, base, base_path, obj_attr, REQUEST)
      if len(s_attr_xml) > 0:
        xml += '\n<lang id="%s"'%langId
  ...
  # Simple Attributes.
    ...
    xml += getAttrToXml( self, base, base_path, obj_attr, REQUEST)
  # Return xml.
  return xml
...
#  _xmllib.getObjToXml:
# ---------------------------------------------------------------------------------------------
def getObjToXml(self, base, REQUEST, deep=True, base_path=''):
  ...
  for key in keys:
    obj_attr = self.getObjAttr(key)
    if obj_attr['xml']:
      ob_prop = getObjPropertyToXml( self, base, base_path, obj_attr, REQUEST)
      if len(ob_prop) > 0:
        xml.append('\n<%s>%s</%s>'%(key,ob_prop,key))
  # Process children.
  if deep:
    xml.extend(map(lambda ob: getObjToXml( ob, base, REQUEST, deep, base_path), self.getChildNodes()))
  # End tag.
  xml.append('</%s>\n'%self.xmlGetTagName())
  # Return xml.
  return "".join(xml)
