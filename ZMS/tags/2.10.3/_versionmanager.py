...
def triggerEvent(self, name, REQUEST):
    ...
    for ob_id in self.getHome().objectIds():
      if ob_id.find( name) == 0:
        try: REQUEST.set('preview','preview')
        except: REQUEST['preview'] = 'preview'
...
class VersionItem: 
      ...
      if checkPending is None:
        ...
        if checkPending:
          ...
          if self.getObjStateNames(REQUEST):
            raise "Can't tagObjVersions: %s@%s has pending changes %s"%(self.meta_type,self.absolute_url(),str(self.getObjStateNames(REQUEST)))
        else:
          #-- Tag master-version.
          obj_version = None
    ...
    def getVersionItems(self, REQUEST, recursive=False):
      ...
      if self.meta_type == 'ZMSCustom':
        ...
        children.extend( self.getObjChildren( '', REQUEST))
      else:
        for child in self.getObjChildren( '', REQUEST):
          if not child.isPage() or child.meta_type in [ 'ZMSLinkElement']:
            children.append( child)
            if not child.isPageElement():
              children.extend( child.getVersionItems( REQUEST))
      return children
    ...
    def isCommitted(self, REQUEST):
      if _globals.isPreviewRequest( REQUEST):
        committed = not self.inObjStates( [ 'STATE_DELETED'], REQUEST) or self.getConfProperty( 'ZMS.workflow.highlightChanges')
      else:
    ...
    # --------------------------------------------------------------------------
    def _commitObjChanges(self, parent, REQUEST, forced=0, do_delete=True):
      if _globals.debug( self):
        _globals.writeLog( self, "[_commitObjChanges]")
      delete = False
      prim_lang = self.getPrimaryLanguage()
      lang = REQUEST.get('lang',prim_lang)
      ...
      if not delete:
        ids = []
        for child in self.getVersionItems( REQUEST):
          delete_child = child._commitObjChanges( self, REQUEST, 0, False)
          if delete_child:
            ids.append( child.id)
        if len( ids) > 0:
      ...
      if (self.getAutocommit() or len( filter( lambda x: x.find( 'STATE_') == 0, getObjStates( self))) == 0) and self.version_live_id is not None:
    ...
    def commitObjChanges(self, parent, REQUEST, forced=0, do_delete=True):
      delete = self._commitObjChanges( parent, REQUEST, forced, do_delete)
      ##### Synchronize catalog. ####
      obs = REQUEST.get('ZMS_SYNCHRONIZE_CATALOG',[])
      for ob in obs:
    ...
    def rollbackObjChanges(self, parent, REQUEST, forced=0, do_delete=True):
      ...
      ##### Synchronize catalog. ####
      for ob in REQUEST.get('ZMS_SYNCHRONIZE_CATALOG',[]):
        ob.synchronizeSearch(REQUEST)
      # Return flag for deleted objects.
      return delete
...
class VersionManagerContainer: 
    ...
    # --------------------------------------------------------------------------
    #  VersionManagerContainer.getVersionManagerContainer
    # --------------------------------------------------------------------------
    def getVersionManagerContainer(self):
      parent = self.getParentNode()
      if (self.meta_type == 'ZMSTeaserContainer') or \
         (self.meta_type == 'ZMSTeaserElement'):
        return parent.getVersionManagerContainer()
      elif (self.meta_type == 'ZMSLinkContainer') or \
    ...
    def autoWfTransition(self, REQUEST):
      ...
      # Special Objects.
      parent = self.getParentNode()
      if (self.meta_type == 'ZMSTeaserElement') or \
         (self.meta_type == 'ZMSTeaserContainer') or \
         (self.meta_type == 'ZMSLinkElement') or \
         (self.meta_type == 'ZMSLinkContainer') or \
         (self.meta_type == 'ZMSCustom' and (parent.meta_type == 'ZMSCustom' or not self.isPage())):
    ...
    #  Commit container.
    # --------------------------------------------------------------------------
    def commitObj(self, REQUEST={}, forced=False):
      if _globals.debug( self): 
      ...
      parent = self.getParentNode()
      delete = self.commitObjChanges(parent,REQUEST,forced)
      url = self.absolute_url()
      if delete: 
