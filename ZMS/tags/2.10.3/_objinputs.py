...
# $Revision: 1.7 $
#
# Implementation of class ObjInputs (see below).
# 
...
class ObjInputs:
  ...
  def getUrlInput(self, fmName, elName, elTextName, size, value, enabled, REQUEST, css='form-element', extra=''):
    ...
    if enabled:
      ...
      if extra.find('onclick') < 0:
        html.append(' onclick="return browseObjsBtnClick(\'%s\',\'%s\',\'%s\')"'%(fmName,elName,elTextName))
      html.append(' %s/>'%extra)
    html.append('</div>')
    ref_obj = self.getLinkObj(value,REQUEST)
    if ref_obj is not None:
      html.append('<div class="form-small">')
      html.append('<b>%s</b>: %s'%(self.getLangStr('ATTR_TARGET',REQUEST['manage_lang']),ref_obj.f_breadcrumbs(objectPathElements=ref_obj.breadcrumbs_obj_path(),no_icon=1,lang=REQUEST['lang'],manage_lang=REQUEST['manage_lang'],REQUEST=REQUEST)))
      html.append('</div>')
    return ''.join(html)
  ...
  def getDateTimeInput(self, fmName, elName, size, value, enabled, fmt_str, REQUEST, css='form-element', extra=''):
    ...
    if value is not None and self.parseLangFmtDate(value) is None:
      value = ''
    html.append('<span class="%s" title="%s">'%(css,self.getLangStr(fmt_str,manage_lang)))
    html.append(self.getTextInput(fmName,elName,size,value,'text',enabled,REQUEST,css,extra))
    if enabled:
      html.append('<a href="javascript:calendarBtnClick(\'%s\',\'%s\',\'%s\')" class="button">'%(fmName,elName,manage_lang))
      html.append('<img src="%sbtn_calendar.gif" title="%s..." border="0" align="middle"/>'%(self.MISC_ZMS,self.getLangStr('ATTR_CALENDAR',manage_lang)))
      html.append('</a>')
    html.append('</span>')
    return ''.join(html)
  ...
  def getTextInput(self, fmName, elName, size=15, value='', type='text', enabled=True, REQUEST=None, css='form-element', extra=''):
    ...
    html.append(' style="width:%iem;"'%size)
    if not enabled:
  ...
  #	@return String
  # ----------------------------------------------------------------------------
  def getCheckbox(self, fmName, elName, value, enabled, hidden, REQUEST, css='form-checkbox', extra=''):
    html = []
    checked = str(value) == '1'
    if elName.find(':int') > 0 and value in [True, False]:
      value = self.boolint(value)
    html.append('<input ')
    html.append(' type="hidden"')
    html.append(' id="%s"'%elName)
    html.append(' name="%s"'%elName)
  ...
  def getTextArea(self, fmName, elName, cols, rows, value, enabled, REQUEST, css='form-element', wrap='virtual', extra=''):
    ...
    html.append(' style="width:%iem;"'%cols)
    if not enabled:
      html.append(' disabled="disabled"')
    html.append('%s>'%extra)
    if value is not None:
    ...
    html.append('</textarea>')
    html.append('</font>')
    if type(value) is type('') and not REQUEST.get('URL','').find('manage_customize') >= 0:
      inline_links = []
      ...
      if len( inline_links) > 0:
        ...
        html.append('<td class="form-small" align="left"><b>%s</b>:</td>'%(self.getLangStr('ATTR_TARGET',REQUEST['manage_lang'])))
        ...
        for c in range( len( inline_links)):
          ...
          if ref_obj is not None:
            html.append('<img src="%sinternal_link.gif" title="" border="0" align="absmiddle"/> %s'%(self.MISC_ZMS,ref_obj.f_breadcrumbs(objectPathElements=ref_obj.breadcrumbs_obj_path(),no_icon=1,lang=REQUEST['lang'],manage_lang=REQUEST['manage_lang'],REQUEST=REQUEST)))
          else:
